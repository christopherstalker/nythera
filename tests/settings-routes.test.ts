...
test("the mobile account hub exposes every desktop settings section", async () => {
  ...
  assert.match(accountHub, /<AccountPreferences \/>/);
  assert.match(accountHub, /SETTINGS_SECTIONS\.map/);
  assert.match(accountHub, /Sign-in details and preferences, all in one place/);
  ...
  assert.equal((sections.match(/href: "\/(?:account|settings\/[^\"]+)"/g) ?? []).length, 10);
  assert.match(sections, /href: "\/settings\/connections"/);
});
