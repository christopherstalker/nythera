...
test("local auth redirects reject protocol-relative and backslash destinations", () => {
  assert.equal(safeLocalRedirect("/settings?tab=security"), "/settings?tab=security");
...
test("the service worker never caches authenticated navigation HTML", async () => {
  ...
  const navigationBranch = serviceWorker.match(/if \(request\.mode === "navigate"\) \{[\s\S]*?\n  \}/)?.[0] ?? "";
...
test("password setup and recovery invalidate older authenticated sessions", async () => {
  const schema = await readFile(new URL("../prisma/schema.prisma", import.meta.url), "utf8");
  const accountRoute = await readFile(new URL("../src/app/api/account/password/route.ts", import.meta.url), "utf8");
  const forgotRoute = await readFile(new URL("../src/app/api/auth/forgot-password/route.ts", import.meta.url), "utf8");
  const resetRoute = await readFile(new URL("../src/app/api/auth/reset-password/route.ts", import.meta.url), "utf8");
...
test("roleplay prompt preserves anatomy without inventing physical limitations", async () => {
  const prompt = await readFile(new URL("../src/lib/prompt-assembly.ts", import.meta.url), "utf8");
  const persona = await readFile(new URL("../src/lib/user-persona-prompt.ts", import.meta.url), "utf8");
  const vector = await readFile(new URL("../src/lib/vector.ts", import.meta.url), "utf8");
  ...
  assert.match(prompt, /The described anatomy is physically present throughout the scene/);
  assert.match(persona, /Player appearance and anatomy:/);
  assert.match(persona, /Player traits:/);
