...
test("gallery stays uniform and increases density across responsive breakpoints", async () => {
  ...
  assert.match(
    globals,
    /@media \(min-width:\s*768px\)[\s\S]*\.codex-character-gallery\s*\{[\s\S]*repeat\(3, minmax\(0, 1fr\)\)/
  );
  assert.match(
    globals,
    /@media \(min-width:\s*1280px\)[\s\S]*\.codex-character-gallery\s*\{[\s\S]*repeat\(4, minmax\(0, 1fr\)\)/
  );
  assert.match(globals, /\.codex-character-plate-image\s*\{[\s\S]*aspect-ratio:\s*5 \/ 4/);
