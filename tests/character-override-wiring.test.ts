...
test("web and mobile character routes persist model overrides", async () => {
  for (const relativePath of [
    ...
    assert.match(source, /preferredProvider:\s*input\.preferredProvider/);
    assert.match(source, /systemPromptOverride:\s*input\.systemPromptOverride/);
    assert.match(source, /maxTokens:\s*input\.maxTokens/);
  }
});
...
test("character system instructions are injected below platform safety", async () => {
  ...
  assert.match(source, /safetyLayer[\s\S]*characterSystemOverrideLayer[\s\S]*personaLayer/);
});
