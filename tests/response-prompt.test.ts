...
test("response instructions are stored and assembled without an arbitrary character limit", async () => {
  ...
  assert.ok(responsePrompt.buildResponsePromptLayer(longInstruction).includes(longInstruction.trim()));
});
...
test("chat persistence and prompt assembly send response instructions through the proxy message payload", async () => {
  ...
  assert.match(validation, /responsePrompt:\s*z\.string\(\)\.trim\(\)\.optional\(\)/);
  assert.doesNotMatch(validation, /responsePrompt:\s*z\.string\(\)\.trim\(\)\.max\(/);
  ...
  assert.doesNotMatch(input, /MAX_RESPONSE_PROMPT_LENGTH/);
});
