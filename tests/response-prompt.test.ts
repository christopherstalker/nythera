...
test("response instructions are constrained to output style below safety and persona authority", async () => {
  const module = await import("../src/lib/response-prompt").catch(() => null);
  assert.ok(module, "response prompt support is missing");
  const layer = module.buildResponsePromptLayer("Write in first person with short dialogue-led replies.");
...
test("response instruction examples give users concise starting points", async () => {
  const module = await import("../src/lib/response-prompt").catch(() => null);
  assert.ok(module, "response prompt support is missing");
  assert.deepEqual(module.RESPONSE_PROMPT_EXAMPLES.map((example) => example.label), ["Cinematic", "Concise", "Dialogue-led"]);
  assert.ok(module.RESPONSE_PROMPT_EXAMPLES.every((example) => example.prompt.length >= 40));
});
