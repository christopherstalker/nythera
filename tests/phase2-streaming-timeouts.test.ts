...
test("provider calls use explicit timeout and abort signals", async () => {
  ...
  assert.match(timeoutSource, /LLM_PROVIDER_TIMEOUT_MS\s*=\s*60_000/);
  assert.match(timeoutSource, /LLM_EMBEDDING_TIMEOUT_MS\s*=\s*15_000/);
  assert.match(gateway, /createTimeoutSignal\(input\.signal,\s*LLM_PROVIDER_TIMEOUT_MS/);
  ...
  assert.match(proxy, /createTimeoutSignal\(input\.signal,\s*LLM_PROVIDER_TIMEOUT_MS/);
  assert.match(proxyRoute, /signal:\s*request\.signal/);
  assert.match(standaloneProxy, /LLM_PROVIDER_TIMEOUT_MS\s*=\s*60_000/);
  assert.match(standaloneProxy, /LLM_EMBEDDING_TIMEOUT_MS\s*=\s*15_000/);
  assert.match(standaloneProxy, /createTimeoutSignal\(clientAbort\.signal,\s*LLM_PROVIDER_TIMEOUT_MS/);
});
