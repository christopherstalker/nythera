...
import { advanceStoryScene, getStoryCodex, updateStoryState } from "@/lib/stories/canon-store";
import { storySceneAdvanceSchema, storyStateSchema } from "@/lib/validation";
...
export async function POST(request: Request, context: Context) {
  try {
    const user = await requireUser();
    await enforceRateLimit({ userId: user.id, ip: getRequestIp(request), route: "stories:state" });
    const input = await parseJson(request, storySceneAdvanceSchema);
    const { previousSceneSummary, carryInventory, ...state } = input;
    return json(await advanceStoryScene({
      storyId: (await context.params).id,
      userId: user.id,
      state,
