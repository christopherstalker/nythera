  ...
  if (!featured && recentHero) {
    return (
      <PageShell className="space-y-8">
        <section className="app-surface relative isolate min-h-[360px] overflow-hidden px-6 py-8 sm:px-8 lg:px-10">
          ...
          <div className="grid min-h-[300px] gap-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center">
            <div className="max-w-3xl">
              ...
              {recentHero.messages[0]?.content ? (
                <p className="mt-5 max-w-xl rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--bg-input)] px-4 py-3 text-sm leading-6 text-[var(--text-secondary)] shadow-[var(--glass-highlight)] backdrop-blur-xl line-clamp-3">
                  {recentHero.messages[0].content}
                </p>
              ) : null}
        ...
        <section>
          <h2 className="px-1 text-xl font-semibold tracking-tight text-[var(--text-primary)]">Recent real characters</h2>
        ...
        <section className="app-surface px-6 py-7 text-center">
          ...
          <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
            The public catalog is still empty because demo characters are hidden. Your private and active chats stay available here.
          </p>
        </section>
      </PageShell>
    );
...
function RecentChatCard({ chat, featured = false }: { chat: RecentChat; featured?: boolean }) {
  return (
    <Link
      href={`/chat/${chat.id}`}
      className="group glass-card glass-card-hover relative flex h-[282px] w-[190px] shrink-0 overflow-hidden no-underline active:scale-[0.98]"
    >
