    ...
    <PageShell className="codex-workspace pb-24">
      <div className="mx-auto max-w-6xl">
        ...
        <PageHeader
          ...
          description="Connect the model providers you already use. Nythera routes character generation and chat through your encrypted provider keys."
        />

        <Surface className="mt-10 border-[var(--codex-violet)]/30 p-6 sm:p-8">
          <p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--codex-violet)]">
            Important distinction
          </p>
          <h2 className="font-editorial mt-2 text-3xl text-[var(--codex-ivory)]">
            Provider APIs, not a public Nythera REST API
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--text-secondary)]">
            This manual explains how to connect OpenAI, Anthropic, Gemini, DeepSeek, xAI, and compatible endpoints.
            Nythera&apos;s internal application routes are not a supported public developer API.
          </p>
        </Surface>
        ...
        <div className="mt-10 grid gap-10">
          <ApiStep icon={KeyRound} number="01" title="Save a provider key">
            <p>
              Open Settings → API Keys, select a provider, paste the key issued by that provider, and save it. The
              browser never receives the stored key again.
            </p>
            <Button asChild className="mt-5">
              <Link href="/settings/providers">Open API Keys</Link>
            </Button>
          </ApiStep>
          ...
          <ApiStep icon={Route} number="03" title="Use fallback routing">
            <p>
              Arrange saved providers, choose an exact model for each step, and move any provider into the primary
              position. Nythera advances past exhausted or invalid credentials and skips directly to the next provider
              during an outage; malformed requests still stop immediately.
            </p>
          </ApiStep>

          <ApiStep icon={Settings2} number="04" title="Connect a custom endpoint">
            <p>
              Use Add custom provider endpoint for an OpenAI-compatible service. Enter a unique provider ID, display
              name, base URL, default model, and API key. The endpoint must implement the OpenAI chat-completions
              request shape.
            </p>
          ...
          <ApiStep icon={LockKeyhole} number="05" title="Security and troubleshooting">
            <ul className="grid gap-3">
              ...
              <li>
                Never paste a key into chat, character text, support requests, screenshots, or public repositories.
              </li>
              ...
              <li>
                For custom endpoints, verify the base URL, model ID, HTTPS certificate, and chat-completions
                compatibility.
              </li>
            </ul>
          </ApiStep>
  ...
  return (
    <section className="grid gap-6 border-t border-[var(--codex-rule)] pt-8 lg:grid-cols-[220px_minmax(0,1fr)]">
      <div>
        ...
        <p className="mt-4 text-xs font-semibold uppercase tracking-[.2em] text-[var(--codex-violet)]">Step {number}</p>
