...
import { hasTextOutput, isTextChatModel } from "@/lib/chat-model-capabilities";
...
export type ProviderCredentialValidation =
  { ok: true; catalog: ProviderModelDiscovery } | { ok: false; message: string; status: number };
  ...
  try {
    ...
    return {
      ...
      status:
        providerStatus === 429
          ? 429
          : providerStatus === 400 || providerStatus === 401 || providerStatus === 403 || providerStatus === 402
            ? 400
            : 502
    };
  }
...
async function fetchLiveCatalog(key: ProviderKey): Promise<ProviderModelDiscovery> {
  ...
  if (key.apiFormat === "GEMINI" || key.provider === "gemini") {
    const payload = await fetchProviderJson("https://generativelanguage.googleapis.com/v1beta/models?pageSize=1000", {
...
async function fetchProviderJson(url: string, headers: Record<string, string>) {
  ...
  if (!response.ok) {
    const error = new Error(`Provider model discovery returned HTTP ${response.status}.`) as Error & {
      status?: number;
    };
...
function uniqueSorted(models: string[]) {
  return Array.from(new Set(models.map((model) => model.trim()).filter(Boolean))).sort((left, right) =>
...
function catalogCacheKey(key: ProviderKey) {
  const fingerprint = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  return `provider-models:v2:${key.provider}:${fingerprint}`;
}
...
function safeDiscoveryWarning(error: unknown) {
  ...
  if (status === 400 || status === 401 || status === 403)
    return "The provider rejected this API key while refreshing models.";
...
function credentialValidationMessage(error: unknown) {
  const status = readErrorStatus(error);
  if (status === 400 || status === 401 || status === 403)
    return "The provider rejected this API key. It was not saved.";
