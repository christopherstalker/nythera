...
const showcaseCharacters: PixelCharacter[] = [
  {
    id: "showcase-raven",
    source: "showcase",
    name: "Raven",
...
export function NytheraPixelExperience({ activeRoute = "/" }: { activeRoute?: string }) {
  ...
  useEffect(() => {
    ...
    void loadExperience().catch(() => undefined);
    return () => controller.abort();
  }, [status]);

  const realCharacters = useMemo(() => characters.map(toPixelCharacter).filter(Boolean) as PixelCharacter[], [characters]);
  ...
  async function startChat(character: PixelCharacter) {
  ...
  return (
    <main className="min-h-dvh overflow-hidden bg-[#03060b] p-3 text-[#f4f2f0] sm:p-5 lg:p-6">
      ...
      >
        <PixelSidebar chats={sidebarChats} activeRoute={activeRoute} />
        <DiscoveryPanel
          query={query}
...
function PixelSidebar({ chats, activeRoute }: { chats: Array<{ name: string; avatar: string; active?: boolean }>; activeRoute: string }) {
  return (
    <aside className="hidden min-h-0 border-r border-white/[0.16] bg-[linear-gradient(180deg,rgba(10,13,20,0.98),rgba(7,10,16,0.94))] px-5 py-6 lg:flex lg:flex-col">
      ...
      <nav className="mt-8 grid gap-3">
        {navItems.map((item) => {
          ...
          const active = item.href === "/" ? activeRoute === "/" : activeRoute === item.href;
          return (
            <Link
              key={item.href}
              href={item.href}
...
function DiscoveryPanel({
  ...
  onStart: (character: PixelCharacter) => void;
}) {
  return (
...
function ChatPreview({ character }: { character: PixelCharacter }) {
  return (
...
function MobileChatPreview({ character }: { character: PixelCharacter }) {
  return (
...
function toPixelCharacter(character: CharacterSummary): PixelCharacter | null {
