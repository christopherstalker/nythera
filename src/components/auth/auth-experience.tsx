  ...
  return (
    <div className="relative mt-10 max-w-lg border-l border-[var(--accent-violet)] pl-5">
      <div className="nythera-auth-dialogue relative">
        ...
        <p
          key={whisper.line}
          className="nythera-auth-whisper-text mt-4 text-2xl font-medium leading-snug text-[var(--text-primary)] sm:text-[1.65rem]"
        >
  ...
  return (
    <div className="nythera-auth-scene codex-auth relative min-h-dvh overflow-hidden">
      <div className="relative z-10 grid min-h-dvh w-full lg:grid-cols-[minmax(0,1.1fr)_minmax(420px,.9fr)]">
        <section className="flex flex-col justify-center border-b border-[var(--border-default)] px-6 py-5 lg:border-b-0 lg:border-r lg:px-[clamp(3rem,7vw,8rem)] lg:py-14">
          ...
          <p className="codex-kicker mt-12 hidden lg:block">{copy.eyebrow}</p>
          <p className="font-editorial mt-5 hidden max-w-2xl text-5xl font-medium leading-[.95] text-[var(--text-primary)] lg:block lg:text-6xl xl:text-7xl">
            {copy.title}
          </p>
          <p className="mt-4 hidden max-w-lg text-sm leading-7 text-[var(--text-secondary)] lg:block lg:text-base">
            {copy.subtitle}
          </p>
          ...
          <ul className="mt-10 hidden gap-3 lg:grid">
            {FEATURES.map((feature) => (
              <li key={feature.label} className="flex items-start gap-3 border-t border-[var(--border-default)] py-4">
        ...
        <section className="flex w-full flex-col justify-center px-6 py-5 lg:px-[clamp(3rem,6vw,7rem)] lg:py-10">
