...
import { TestSceneDialog } from "@/components/studio/test-scene-dialog";
...
export function CreatorStudioClient() {
  ...
  const [testCharacter, setTestCharacter] = useState<StudioCharacter | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([fetch("/api/profile", { cache: "no-store" }), fetch("/api/library", { cache: "no-store" })])
  ...
  if (sessionStatus === "unauthenticated")
    return <p className="p-6 text-sm text-[var(--text-secondary)]">Sign in to open Creator Studio.</p>;
      ...
      <header className="relative overflow-hidden border-b border-[var(--codex-rule)] px-5 pb-8 pt-6 sm:px-0 sm:pt-2">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-3 font-editorial text-5xl leading-none text-[var(--text-primary)] sm:text-6xl">
              Your worlds, in one place.
            </h1>
            <p className="mt-4 max-w-xl text-sm leading-6 text-[var(--text-secondary)]">
              Build your creator identity, publish characters, and see what your audience is actually engaging with.
            </p>
          </div>
      ...
      <main className="grid min-w-0 gap-8 px-5 pt-8 sm:px-0 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 space-y-8">
          <section
            ...
            className="grid grid-cols-2 gap-px overflow-hidden border border-[var(--codex-rule)] bg-[var(--codex-rule)] lg:grid-cols-4"
          >
          ...
          <section aria-labelledby="studio-characters-title">
            <div className="flex flex-col gap-4 border-b border-[var(--codex-rule)] pb-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h2 id="studio-characters-title" className="mt-1 text-2xl font-semibold text-[var(--text-primary)]">
                  Characters
                </h2>
              </div>
        ...
        <aside className="min-w-0 space-y-4 xl:sticky xl:top-6 xl:self-start">
          <section className="border border-[var(--codex-rule)] bg-[var(--codex-paper-raised)] p-5">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 truncate text-xs text-[var(--text-muted)]">
                  {profile?.username ? `@${profile.username}` : "Claim your public address"}
                </p>
              </div>
            </div>
            ...
            <form onSubmit={saveUsername} className="mt-5 space-y-3">
              <label
                htmlFor="studio-username"
                className="text-[10px] font-medium uppercase tracking-[.14em] text-[var(--text-muted)]"
              >
                Unique username
              </label>
              <UsernameField
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={!usernameChanged || Boolean(usernameError) || usernameAvailable !== true || savingUsername}
              >
                {savingUsername ? "Saving…" : profile?.username ? "Update address" : "Claim username"}
              </Button>
              {statusMessage ? (
                <p role="status" className="text-xs leading-5 text-[var(--text-secondary)]">
                  {statusMessage}
                </p>
              ) : null}
            </form>
          ...
          <section className="border border-[var(--codex-rule)] p-5">
            ...
            <div className="mt-5 h-1.5 overflow-hidden rounded-full bg-[var(--codex-rule)]">
              <div
                className="h-full bg-[var(--codex-mint)] transition-[width]"
                style={{
                  width: `${([Boolean(profile?.username), Boolean(profile?.avatarUrl), Boolean(profile?.bio?.trim()), totals.publicCount > 0].filter(Boolean).length / 4) * 100}%`
                }}
              />
            </div>
            <p className="mt-2 text-xs text-[var(--text-muted)]">
              {profileReady && totals.publicCount > 0
                ? "Your creator profile is ready for visitors."
                : "Complete the basics to make your page feel trustworthy."}
            </p>
          </section>
        </aside>
      </main>
  ...
  return (
    <div className="bg-[var(--codex-paper-raised)] p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3 text-[var(--text-muted)]">
        <span className="text-[10px] uppercase tracking-[.14em]">{label}</span>
...
function StudioCharacterCard({ character, onTest }: { character: StudioCharacter; onTest: () => void }) {
  const updatedLabel = new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(
    ...
    <article className="group min-w-0 border border-[var(--codex-rule)] bg-[var(--codex-paper-raised)] p-4 transition-colors hover:border-[var(--border-strong)]">
      <div className="flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-3 line-clamp-2 break-words text-xs leading-5 text-[var(--text-secondary)]">
            {character.description || "No public description yet."}
          </p>
        </div>
      </div>
      ...
      <Button type="button" variant="secondary" className="mt-3 w-full rounded-lg" onClick={onTest}>
...
function VisibilityBadge({ visibility, moderationStatus }: Pick<StudioCharacter, "visibility" | "moderationStatus">) {
  ...
  return (
    <span
      className={cn(
        "mt-2 inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-[9px] uppercase tracking-[.12em]",
        visibility === "PUBLIC" && !needsReview
          ? "border-emerald-400/40 text-emerald-300"
          : "border-[var(--codex-rule)] text-[var(--text-muted)]"
      )}
    >
      <span
        className={cn(
...
function EmptyStudio() {
  return (
    <div className="flex flex-col items-center border border-dashed border-[var(--codex-rule)] px-6 py-16 text-center">
      <span className="grid h-16 w-16 place-items-center rounded-full border border-[var(--codex-rule)] text-[var(--codex-mint)]">
      ...
      <p className="mt-2 max-w-sm text-sm leading-6 text-[var(--text-secondary)]">
        Start with a spark or build every detail yourself. You can keep the character private until it is ready.
      </p>
...
function StudioSkeleton() {
  return (
    <div className="space-y-6 px-5 py-8 sm:px-0">
...
function StudioError({ message }: { message: string }) {
  return (
    <div className="mx-5 my-8 flex items-start gap-3 border border-rose-400/30 bg-rose-400/5 p-5 text-sm text-rose-200 sm:mx-0">
      <CircleAlert className="mt-0.5 h-5 w-5 shrink-0" />
      <div>
        <p className="font-medium">Creator Studio is unavailable</p>
        <p className="mt-1 text-rose-200/75">{message}</p>
      </div>
    </div>
  );
}
