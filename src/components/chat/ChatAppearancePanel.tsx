...
const fieldClass =
  "focus-ring h-11 w-full rounded-sm border border-[var(--border-default)] bg-[var(--bg-input)] px-3 text-sm text-[var(--text-primary)]";
    ...
    <div className="grid gap-6">
      ...
      <section className="grid gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
            The character portrait is used by default. Upload an image, GIF, MP4, WEBM, or MOV for this conversation.
          </p>
        </div>
        ...
        <label
          htmlFor={fileInputId}
          className="focus-ring flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-sm border border-dashed border-[var(--border-strong)] px-4 text-xs font-semibold uppercase tracking-[.1em] text-[var(--text-secondary)] hover:border-[var(--codex-mint)] hover:text-[var(--codex-mint)]"
        >
        ...
        <div className="grid grid-cols-2 gap-2">
          <SelectField
            ...
            onChange={(value) => update("backgroundFit", value as ChatAppearance["backgroundFit"])}
            options={["cover", "contain"]}
          />
          <SelectField
            label="Position"
            value={draft.backgroundPosition}
            onChange={(value) => update("backgroundPosition", value as ChatAppearance["backgroundPosition"])}
        ...
        <RangeField
          ...
          max={0.92}
          step={0.01}
          display={`${Math.round(draft.backgroundDim * 100)}%`}
          onChange={(value) => update("backgroundDim", value)}
        />
        <RangeField
          ...
          min={0}
          max={24}
          display={`${draft.backgroundBlur}px`}
      ...
      <section className="grid gap-3 border-t border-[var(--border-default)] pt-5">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
            Changes appear in the conversation immediately. Any installed font family can be entered manually.
          </p>
        </div>
        <label className="grid gap-1.5 text-xs text-[var(--text-secondary)]">
          Font preset
          <select
            className={fieldClass}
            value={
              draft.fontUrl || draft.fontFamily === CHAT_CUSTOM_FONT_FAMILY
                ? "custom"
        ...
        <label
          htmlFor={fontInputId}
          className="focus-ring flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-sm border border-dashed border-[var(--border-strong)] px-4 text-xs font-semibold uppercase tracking-[.1em] text-[var(--text-secondary)] hover:border-[var(--codex-mint)] hover:text-[var(--codex-mint)]"
        >
        ...
        <label className="grid gap-1.5 text-xs text-[var(--text-secondary)]">
          ...
          <input
            className={fieldClass}
            value={draft.fontUrl ? "" : draft.fontFamily}
            onChange={(event) =>
              setStoredAppearance(normalizeChatAppearance({ ...draft, fontFamily: event.target.value, fontUrl: "" }))
            }
            placeholder="Font family name"
            disabled={Boolean(draft.fontUrl)}
          />
        </label>
        <div className="grid grid-cols-[1fr_88px] gap-2">
          ...
          <label className="grid gap-1.5 text-xs text-[var(--text-secondary)]">
            ...
            <input
              type="color"
              className="h-11 w-full cursor-pointer rounded-sm border border-[var(--border-default)] bg-[var(--bg-input)] p-1"
        ...
        <RangeField
          ...
          max={800}
          step={100}
          display={String(draft.fontWeight)}
      ...
      <section className="grid gap-3 border-t border-[var(--border-default)] pt-5">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
            Attach a YouTube Music, Spotify, SoundCloud, or Apple Music link. Playback starts only after the listener
            taps play.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 border-t border-[var(--border-default)] pt-5">
        <Button
          ...
          disabled={saving || uploading || uploadingFont}
        >
        ...
        <Button
          ...
          disabled={!activeChatId || saving || uploading || uploadingFont}
        >
      ...
      {status ? (
        <p
          role="status"
          className="border-l-2 border-[var(--codex-mint)] pl-3 text-xs leading-5 text-[var(--text-secondary)]"
        >
          {status}
        </p>
      ) : null}
...
function SelectField({
  ...
  return (
    <label className="grid gap-1.5 text-xs capitalize text-[var(--text-secondary)]">
      {label}
...
function RangeField({
  ...
  return (
    <label className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1.5 text-xs text-[var(--text-secondary)]">
      <span>{label}</span>
      <span className="tabular-nums text-[var(--text-primary)]">{display}</span>
      <input
        className="col-span-2 w-full accent-[var(--codex-mint)]"
        type="range"
        min={min}
        max={max}
        step={step}
