...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  if (hideChrome) {
    return (
      <main className="h-dvh touch-pan-y overflow-y-auto overscroll-y-contain bg-[var(--bg-base)]">{children}</main>
    );
  }
    ...
    >
      ...
      >
        <main
          ...
          ref={mainRef}
          className={cn(
            "codex-main min-h-0 min-w-0 max-w-full flex-1 overflow-y-auto",
            isImmersiveSurface && "overflow-hidden"
          )}
        >
          {children}
        </main>
      ...
      {!isChatSurface ? (
        <>
          <div
            aria-hidden
            className="h-[calc(var(--codex-mobile-dock-height)+env(safe-area-inset-bottom))] md:hidden"
          />
          <MobileDock />
        </>
      ) : null}
