...
export function KeySettingsClient({
  ...
  const refreshModels = useCallback(async (force: boolean) => {
    ...
    try {
      ...
      const availableModelCount = new Set(
        entries.flatMap((entry) => entry.models.map((model) => `${entry.provider}:${model}`))
      ).size;
      setStatus(
        warning ??
  ...
  useEffect(() => {
    ...
    const refreshIfStale = () => {
      if (
        ...
        Date.now() - lastCatalogRefreshRef.current >= MODEL_CATALOG_REFRESH_INTERVAL_MS
      ) {
  ...
  const orderedProviders = useMemo(
    () =>
      [...providers].sort(
        (left, right) => Number(savedByProvider.has(right.provider)) - Number(savedByProvider.has(left.provider))
      ),
    [savedByProvider]
  );
  ...
  const fallbackKeys = useMemo(
    () => Array.from(savedByProvider.values()).map((providerKeys) => providerKeys[0]),
    [savedByProvider]
  );
  const fallbackModels = useMemo(() => {
    ...
    for (const key of fallbackKeys) {
      ...
      modelsByProvider.set(key.provider, modelSuggestionsForProvider(key.provider, key.defaultModel, discovered));
    }
    return modelsByProvider;
  }, [fallbackKeys, modelCatalog]);
  const customError = validateCustomProvider(custom);
  const customStarted = Boolean(
    custom.provider || custom.displayName || custom.baseUrl || custom.defaultModel || custom.apiKey || custom.label
  );
  ...
  function toggleFallback(provider: string) {
    setKeys((current) =>
      current.map((key) =>
        key.provider === provider && !key.isDefault ? { ...key, fallbackEnabled: !key.fallbackEnabled } : key
      )
    );
  }

  function selectFallbackModel(provider: string, model: string) {
    setKeys((current) => current.map((key) => (key.provider === provider ? { ...key, defaultModel: model } : key)));
  }
    ...
    try {
      ...
      setStatus(
        body.maxOutputTokens
          ? `Global ceiling saved: ${body.maxOutputTokens.toLocaleString()} tokens. A lower character limit still applies.`
          : "Global token limit removed."
      );
    } catch {
    ...
    <div className="grid gap-4">
      ...
      {onboarding ? (
        ...
        >
          ...
          <h3 id="provider-onboarding-title" className="mt-2 font-editorial text-3xl text-[var(--text-primary)]">
            Connect a model before you enter a story.
          </h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
            Nythera does not bundle a paid model account. Add one provider key below; it is verified before it is
            stored.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            {verifiedKeyCount > 0 ? (
              <Button asChild>
                <Link href={callbackUrl}>Continue to Nythera</Link>
              </Button>
            ) : (
      ...
      <details className="studio-provider-card glass-card">
        <summary>
          Response length <span>Optional token limit</span>
        </summary>
        <form onSubmit={saveOutputTokenLimit} className="p-4" aria-labelledby="output-token-limit-title">
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(180px,240px)_auto] sm:items-end">
            <div>
              <h3 id="output-token-limit-title" className="text-sm font-semibold text-[var(--text-primary)]">
                Maximum output tokens
              </h3>
              <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
                Leave it empty for no global token limit. Short, Medium, and Long guide the writing style without
                setting a token cap. An explicit character limit and the provider&apos;s limits still apply. When a
                limit is set, reasoning uses part of that budget on thinking models.
              </p>
            </div>
      ...
      <div className="glass-card flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
            Available models refresh automatically from each connected provider. Bundled models are used only if a
        ...
        <section className="glass-card p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
                Move any provider to the first position to make it primary, then choose the exact model for every step.
                Failed keys advance to the next saved key; invalid requests still stop immediately.
              </p>
            </div>
          </div>
          <div className="mt-4 grid gap-2">
            {fallbackKeys.map((key, index) => (
              <div
                key={key.provider}
                className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-input)] p-3"
              >
                <input
                  type="checkbox"
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-[var(--text-primary)]">
                    ...
                    {rejectedProviders.has(key.provider) ? " · invalid key" : ""}
                  </p>
                  <select
                  ...
                  >
                    {(
                      fallbackModels.get(key.provider) ?? [
                        key.defaultModel?.trim() || defaultModelForProvider(key.provider)
                      ]
                    ).map((model) => (
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="icon"
                    aria-label={`Move ${key.displayName} up`}
                    onClick={() => moveFallback(index, -1)}
                    disabled={index === 0 || (index === 1 && rejectedProviders.has(key.provider))}
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="icon"
                    aria-label={`Move ${key.displayName} down`}
                    onClick={() => moveFallback(index, 1)}
      ...
      {orderedProviders.map((provider) => {
        ...
        return (
          <details key={provider.provider} className="studio-provider-card glass-card">
            <summary>
              <span>{provider.displayName}</span>
              <span>
                {savedKeys.length
                  ? `${savedKeys.length} connected ${savedKeys.length === 1 ? "key" : "keys"}`
                  : "Not connected"}
              </span>
            </summary>
            <form onSubmit={(event) => saveProvider(event, provider.provider)} className="p-4">
                ...
                <div>
                  ...
                  {liveCatalog ? (
                    <div className="mt-1 text-xs text-[var(--codex-mint)]">
                      <p>
                        {liveCatalog.models.length} models · live catalog ·{" "}
                        {formatCatalogRefresh(liveCatalog.refreshedAt)}
                      ...
                      <details className="mt-2 text-[var(--text-muted)]">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {liveCatalog.models.map((model) => (
                            <code
                              key={model}
                              className="rounded-md border border-[var(--border-default)] bg-[var(--bg-input)] px-2 py-1 text-[11px] text-[var(--text-secondary)]"
                            >
                              {model}
                            </code>
                          ))}
                ...
                <div className="mt-4 grid gap-2" aria-label={`${provider.displayName} failover keys`}>
                  {savedKeys.map((key, index) => {
                    ...
                    return (
                      <div
                        key={key.id}
                        className="flex items-center gap-3 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-input)] p-3 shadow-[var(--glass-highlight)]"
                      >
                        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[var(--border-default)] text-xs font-semibold text-[var(--text-secondary)]">
                          {index + 1}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium text-[var(--text-primary)]">{keyName}</p>
                          <p
                            className={`mt-1 truncate text-xs ${rejected ? "text-amber-300" : "text-[var(--text-muted)]"}`}
                          >
                            {rejected ? "Invalid" : key.credentialStatus === "VALID" ? "Verified" : "Unverified"} ·
                            ending in {key.last4}
                          </p>
                          {catalog?.warning ? (
                            <p className="mt-1 text-xs leading-5 text-amber-300" role="status">
                              {catalog.warning}
                            </p>
                          ) : null}
                        </div>
              ...
              <Input
                ...
                aria-label={`${provider.displayName} key label`}
                maxLength={80}
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
                <Input
                  value={value}
                  onChange={(event) =>
                    setValues((current) => ({ ...current, [provider.provider]: event.target.value }))
                  }
                ...
                <Button
                  ...
                  size="icon"
                  aria-label="Copy key"
                  onClick={() => copy(provider.provider)}
                  disabled={!value}
                >
          ...
          <div className="mt-3 grid gap-2">
            {keys
              ...
              .map((key) => (
                <div
                  key={key.id}
                  className="flex items-center justify-between gap-3 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-input)] p-3 shadow-[var(--glass-highlight)]"
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[var(--text-primary)]">
                      {key.label || key.displayName}
                    </p>
                    <p className="mt-1 truncate text-xs text-[var(--text-muted)]">
                      {key.displayName} · {key.provider} · {key.defaultModel || "no default model"} · ending in{" "}
                      {key.last4}
                    </p>
                  </div>
      ...
      <details className="studio-provider-card glass-card">
        ...
        <form onSubmit={saveCustom} className="p-4">
          ...
          {customStarted && customError ? (
            <p className="mt-3 text-xs leading-5 text-amber-300" role="alert">
              {customError}
            </p>
          ) : null}
          {formErrors.custom ? (
